...
export function UserProjectsOverview({ userId }: UserProjectsOverviewProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/user/${userId}/project-task`);
        const json = await res.json();
        setOwnedProjects(json.ownedProjects);
        setAssignedTasks(json.assignedTasks);
      } catch (err) {
        console.error("Failed to load user project data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Projects you own and tasks assigned to you
        </CardDescription>
      </CardHeader>
      <CardContent>
