...
export function ProjectProgress({ projectId }: ProjectProgressProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`/api/project/${projectId}/task-summary`);
        setData([
          { name: "Completed", value: res.data.completed, color: "#22c55e" },
          { name: "In Progress", value: res.data.inProgress, color: "#3b82f6" },
          { name: "Not Started", value: res.data.notStarted, color: "#6b7280" },
        ]);
      } catch (error) {
